-- Chốn Học Tập: diễn đàn V15.
-- Ảnh giữ nguyên độ phân giải; video được trình duyệt nén về tối đa 720p trước khi lưu.
-- Áp dụng giống nhau cho media của bài viết và bình luận.
...
begin
  ...
  if tg_table_name = 'forum_post_media' then
    ...
    into image_count, video_count, audio_count, image_video_total
  ...
  -- Ảnh được phép giữ nguyên mọi độ phân giải. Video phải là kết quả tối đa 720p.
  ...
  if uploader_role = 'member' then
    ...
    if new.media_type = 'image' and new.size_bytes > 52428800 then
      raise exception 'Mỗi ảnh thành viên tối đa 50 MB và được giữ nguyên độ phân giải';
    end if;
    if new.media_type = 'video'
      and (new.size_bytes > 52428800 or new.duration_seconds > 60) then
      raise exception 'Video thành viên tối đa 50 MB, 720p và 1 phút';
    end if;
    ...
    if new.media_type = 'video'
      and (new.size_bytes > 52428800 or new.duration_seconds > 60) then
      raise exception 'Video VIP/điều hành viên tối đa 50 MB, 720p và 1 phút';
    end if;
  end if;
...
comment on function public.validate_forum_media_limits() is
  'Ảnh giữ nguyên độ phân giải; video tối đa 720p; Member có tổng ảnh/video 50 MB cho mỗi bài hoặc bình luận.';
